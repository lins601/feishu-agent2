...
# -*- coding: utf-8 -*-
"""
MinDoc -> Markdown -> WISE 知识库入库脚本 V4 (定时增量同步版)

核心能力：
1. 并发爬取（ThreadPoolExecutor）
2. HTTP 会话复用 + 连接池（urllib3）
3. 按 md_hash 增量判断文档是否更新
4. 未变化文档跳过，新增文档上传，更新文档重新上传
5. 更新文档在新版本解析完成后删除 WISE 旧知识
6. 分批保存映射表，支持定时任务每月执行
    ...
    *,
    ...
    for link in extractor.links:
        ...
        if include_path and include_path not in parsed.path:
            continue
        if exclude_path and exclude_path in parsed.path:
            continue
        if norm in seen:
            continue
...
class CrawledDoc:
    ...
    old_wise_knowledge_id: str = ""
    delete_old_status: str = ""
    delete_old_error: str = ""
    parse_status: str = ""
    error_message: str = ""
    crawl_time: str = ""
    ...
    def to_mapping_dict(self) -> dict[str, Any]:
        return {
            ...
            "old_wise_knowledge_id": self.old_wise_knowledge_id,
            "delete_old_status": self.delete_old_status,
            "delete_old_error": self.delete_old_error,
...
class MinDocCrawler:
    ...
    def crawl_tree_concurrent(
        ...
        *,
        ...
        print(f"   exclude:  {exclude_path or '(未限制)'}")
        ...
        with ThreadPoolExecutor(max_workers=self.concurrency) as executor:
            ...
            for future in as_completed(futures):
                ...
                # 检查是否需要跳过 / 更新 / 新增
                # document_key 由 project_name + normalized_url hash 组成，用于定位同一篇文档；
                # md_hash 由正文 Markdown 计算，用于判断内容是否变化。
                old = self.mapping.get(doc.document_key)
                if old and old.get("md_hash") == doc.md_hash and old.get("wise_knowledge_id"):
                    doc.sync_status = "skipped"
                    doc.wise_knowledge_id = old.get("wise_knowledge_id", "")
                    doc.old_wise_knowledge_id = old.get("old_wise_knowledge_id", "")
                    ...
                    # 这里先保存旧 WISE 知识 ID，上传新版本成功后再删除旧版本。
                    doc.old_wise_knowledge_id = old.get("wise_knowledge_id", "")
                    doc.wise_knowledge_id = old.get("wise_knowledge_id", "")
                    stats["updated"] += 1
                else:
                    doc.sync_status = "new"
...
class WiseClient:
    ...
    def delete_knowledge(self, knowledge_id: str) -> dict[str, Any]:
        ...
        默认按当前脚本已有的 knowledge detail 路径使用 DELETE /knowledge/{id}。
        如果你们 WISE 环境的删除接口路径不同，只需要改这里。
        """
        url = self._api_url(f"/knowledge/{knowledge_id}")
        return self._curl_request(url, method="DELETE", headers=self._headers_json(), timeout=120)
...
def upload_doc_to_wise(
    ...
    *,
    ...
    poll: bool = True,
    delete_old_on_update: bool = False,
) -> CrawledDoc:
    metadata = {
        "source": str(SOURCE),
    ...
    # 如果是更新文档，doc.wise_knowledge_id 里暂时保存的是旧知识 ID；
    # 先上传新版本，确认新版本可用后，再删除旧版本，避免知识库短时间空窗。
    old_knowledge_id = doc.old_wise_knowledge_id or (doc.wise_knowledge_id if doc.sync_status == "updated" else "")
    ...
    doc.old_wise_knowledge_id = old_knowledge_id
    ...
    # 更新文档：新版本上传成功，且解析完成后再删除旧版本。
    # 如果关闭轮询，则只能在上传成功后删除旧版本。
    should_delete_old = (
        delete_old_on_update
        and old_knowledge_id
        and old_knowledge_id != knowledge_id
        and (not poll or doc.parse_status == "completed")
    )
    if should_delete_old:
        delete_result = client.delete_knowledge(old_knowledge_id)
        if delete_result.get("success") is False:
            doc.delete_old_status = "failed"
            doc.delete_old_error = str(delete_result.get("error") or delete_result)
            log.warning("删除旧知识失败: old_id=%s, error=%s", old_knowledge_id, doc.delete_old_error)
        else:
            doc.delete_old_status = "deleted"
    elif delete_old_on_update and old_knowledge_id and old_knowledge_id != knowledge_id:
        doc.delete_old_status = "skipped"
        if poll and doc.parse_status != "completed":
            doc.delete_old_error = f"新知识 parse_status={doc.parse_status}，为避免知识空窗，暂不删除旧知识"
...
def upload_docs_concurrent(
    ...
    def upload_one(doc: CrawledDoc) -> CrawledDoc:
        if not force and doc.sync_status == "skipped" and doc.wise_knowledge_id:
            return doc
        return upload_doc_to_wise(
            doc,
            client,
            inject_images=inject_images,
            poll=poll,
            delete_old_on_update=delete_old_on_update,
        )
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        ...
        for future in as_completed(futures):
            ...
            if doc.sync_status == "upload_failed":
                ...
                delete_info = f" | 旧知识: {doc.delete_old_status}" if doc.delete_old_status else ""
                print(f"📤 ✅ {doc.title} | id={doc.wise_knowledge_id} | 图片: {ok_images}/{ok_images+bad_images}{delete_info}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--exclude", default="", help="排除路径包含内容")
    ...
    parser.add_argument("--delete-old-on-update", action="store_true", help="更新文档上传新版本解析完成后，删除 WISE 旧知识")
...
def main() -> int:
    ...
    print("\n" + "=" * 72)
    print("MinDoc -> Markdown -> WISE 知识库入库 V4 (定时增量同步版)")
    ...
    uploaded, failed, img_ok, img_fail = upload_docs_concurrent(
        ...
        delete_old_on_update=args.delete_old_on_update,
    )
