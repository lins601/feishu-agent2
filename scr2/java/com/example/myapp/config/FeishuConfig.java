...
public class FeishuConfig {
    ...
    public EventDispatcher eventDispatcher(FeishuMessageService feishuMessageService,
        ...
        boolean handleMessageEvents = !wiseOnlyFeedbackMode || listenMessageEvents;
        if (handleMessageEvents) {
            ...
            log.info("WISE-only 模式下不订阅消息接收事件，仅通过轮询检测 WISE 回复");
        }
    ...
    private void handleReceiveMessage(P2MessageReceiveV1 event, FeishuMessageService service,
        ...
        try {
            ...
            if ("text".equals(msgType)) {
                pollingService.scheduleFeedbackCard(chatId, messageId, textContent, createTime);
            }
